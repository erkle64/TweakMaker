	...
	public class DarkModeCS
	{
		...
		public void ThemeControl(Control control)
		{
					...
					using (Brush tabBack = new SolidBrush(OScolors.Surface))
					{
						for (int i = 0; i < tab.TabPages.Count; i++)
						{
							...
							tabPage.ControlAdded += (object _s, ControlEventArgs _e) =>
							{
                                if (_s is null)
                                {
                                    throw new ArgumentNullException(nameof(_s));
                                }
			...
			if (control is ListView listView)
			{
				...
				listView.DrawColumnHeader += (sender, e) => ListView_DrawColumnHeader(sender, e, OScolors);
        ...
        private void ListView_DrawColumnHeader(object? sender, DrawListViewColumnHeaderEventArgs e, OSThemeColors oScolors)
        {
            using (SolidBrush backBrush = new SolidBrush(OScolors.Control))
            {
                e.Graphics.FillRectangle(backBrush, e.Bounds);
            }
            using (Pen backBrush = new Pen(Color.Black))
            {
